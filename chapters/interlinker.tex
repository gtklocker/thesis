...
What we ended up using is busy polling through JSON-RPC, using the \code{getbestblockhash} method. Every 5 seconds, or any defined interval in the configuration, the interlinker will check the best block hash and if it doesn't match the one previously given then this means that there's been a new block.
